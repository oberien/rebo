...
use crate::parser::{ExprFieldAccess, ExprAssign, ExprAssignLhs, ExprVariable, Spanned};
...
fn check_non_struct_field_access(diagnostics: &Diagnostics, meta_info: &MetaInfo, expr: &ExprFieldAccess) {
    ...
    let mut typ = meta_info.types[&TypeVar::new(variable.binding.span())].clone();
    let mut span = variable.span();
    for field in fields {
        let struct_name = match &typ {
            Type::Top | Type::Bottom => return,
            Type::Specific(SpecificType::Struct(_, name)) => name,
            _ => {
        ...
        let field_type = struct_type.fields.iter()
            .filter_map(|(name, typ)| if name == field.ident { Some(typ) } else { None })
            .next();
        match field_type {
            Some(field_typ) => {
