...
impl Visitor for ImmutableAssign {
    ...
    fn visit_add_assign(&self, diagnostics: &Diagnostics, _: &MetaInfo, assign: &ExprAddAssign) {
        check_mutable_assign(&assign.lhs, diagnostics);
    }
    fn visit_sub_assign(&self, diagnostics: &Diagnostics, _: &MetaInfo, assign: &ExprSubAssign) {
        check_mutable_assign(&assign.lhs, diagnostics);
    }
    fn visit_mul_assign(&self, diagnostics: &Diagnostics, _: &MetaInfo, assign: &ExprMulAssign) {
        check_mutable_assign(&assign.lhs, diagnostics);
    }
    fn visit_div_assign(&self, diagnostics: &Diagnostics, _: &MetaInfo, assign: &ExprDivAssign) {
        check_mutable_assign(&assign.lhs, diagnostics);
    }
    fn visit_bool_and_assign(&self, diagnostics: &Diagnostics, _: &MetaInfo, assign: &ExprBoolAndAssign) {
        check_mutable_assign(&assign.lhs, diagnostics);
    }
    fn visit_bool_or_assign(&self, diagnostics: &Diagnostics, _: &MetaInfo, assign: &ExprBoolOrAssign) {
        check_mutable_assign(&assign.lhs, diagnostics);
    }
}
