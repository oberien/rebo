...
impl Visitor for ImmutableAssign {
    fn visit_assign(&self, diagnostics: &Diagnostics, _: &MetaInfo, _: &BlockStack<'_, '_, ()>, assign: &ExprAssign) {
        check_mutable_assign(&assign.lhs, diagnostics);
    }
    fn visit_add_assign(&self, diagnostics: &Diagnostics, _: &MetaInfo, _: &BlockStack<'_, '_, ()>, assign: &ExprAddAssign) {
        check_mutable_assign(&assign.lhs, diagnostics);
    }
    fn visit_sub_assign(&self, diagnostics: &Diagnostics, _: &MetaInfo, _: &BlockStack<'_, '_, ()>, assign: &ExprSubAssign) {
        check_mutable_assign(&assign.lhs, diagnostics);
    }
    fn visit_mul_assign(&self, diagnostics: &Diagnostics, _: &MetaInfo, _: &BlockStack<'_, '_, ()>, assign: &ExprMulAssign) {
        check_mutable_assign(&assign.lhs, diagnostics);
    }
    fn visit_div_assign(&self, diagnostics: &Diagnostics, _: &MetaInfo, _: &BlockStack<'_, '_, ()>, assign: &ExprDivAssign) {
        check_mutable_assign(&assign.lhs, diagnostics);
    }
    fn visit_bool_and_assign(&self, diagnostics: &Diagnostics, _: &MetaInfo, _: &BlockStack<'_, '_, ()>, assign: &ExprBoolAndAssign) {
        check_mutable_assign(&assign.lhs, diagnostics);
    }
    fn visit_bool_or_assign(&self, diagnostics: &Diagnostics, _: &MetaInfo, _: &BlockStack<'_, '_, ()>, assign: &ExprBoolOrAssign) {
        check_mutable_assign(&assign.lhs, diagnostics);
    }
}

fn check_mutable_assign(lhs: &ExprAssignLhs, diagnostics: &Diagnostics) {
